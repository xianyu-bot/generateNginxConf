...
class Formatter:
    ...
    def format_file(self,
        ...
        chosen_encoding, original_file_content = self._load_file_content(
            file_path)
        ...
        if original_backup_file_path:
            ...
            self.logger.info("Original content saved to '%s'.",
                             original_backup_file_path)
    ...
    def _load_file_content(self,
        ...
        self.logger.info(
            "Loaded file '%s' (detected encoding %s).", file_path, chosen_encoding)
    ...
    def _apply_variable_template_tags(self, line: str) -> str:
        ...
        return re.sub(r'\${\s*(\w+)\s*}',
                      self._TEMPLATE_VARIABLE_OPENING_TAG + r"\1" +
                      self._TEMPLATE_VARIABLE_CLOSING_TAG,
                      line,
                      flags=re.UNICODE)
        ...
        for line in orig_lines:
            ...
            if line == "":
                ...
                if q == 1 and c > 1:
                    ...
                    newlines = line.split(";")
                    lines_to_add = self._clean_lines(
                        ["".join([ln, ";"]) for ln in newlines if ln != ""])
                    cleaned_lines.extend(lines_to_add)
                else:
                    if line.startswith("rewrite"):
                        cleaned_lines.append(
                            self._strip_variable_template_tags(line))
                    else:
...
def _standalone_run(program_arguments):
    arg_parser = argparse.ArgumentParser(
        description="Formats nginx configuration files in consistent way.")

    arg_parser.add_argument(
        "-v", "--verbose", action="store_true", help="show formatted file names")
    ...
    formatter_options_group = arg_parser.add_argument_group(
        "formatting options")
    ...
    try:
        if args.pipe and len(args.config_files) != 0:
            raise Exception(
                "if %s is enabled, no file can be passed as input" % _aname(pipe_arg))
        if args.pipe and args.backup_original:
            raise Exception(
                "cannot create backup file when %s is enabled" % _aname(pipe_arg))
        if args.print_result and len(args.config_files) > 1:
            raise Exception(
                "if %s is enabled, only one file can be passed as input" % _aname(print_result_arg))
        if len(args.config_files) == 0 and not args.pipe:
            raise Exception(
                "no input files provided, specify at least one file or use %s" % _aname(pipe_arg))
